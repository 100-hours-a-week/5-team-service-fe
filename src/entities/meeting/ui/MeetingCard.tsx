...
import { BookOpenIcon, ClockIcon, FaceFrownIcon } from "@heroicons/react/24/solid";
...
function MeetingCardBase({ meeting, genreName, onClick, onImpression }: MeetingCardProps) {
  ...
  return (
    ...
    >
      ...
      {!isRecruiting ? (
        <div className="flex flex-col gap-2 pointer-events-none absolute inset-0 z-10 flex items-center justify-center bg-gray-800/50">
          <FaceFrownIcon className="h-10 w-10 text-white" />
          <p className="px-4 text-center text-body-2 text-white">모집이 종료된 모임입니다.</p>
        </div>
      ) : null}
    </Link>
  );
